...
## V19 production checklist

1. Run `supabase/schema_v15_migration.sql` after the existing migrations.
2. Verify the `student_active_sessions` table exists and has a unique `student_id` constraint.
3. Verify Storage bucket limits from the v15 migration.
4. Confirm `JWT_SECRET`, `SUPABASE_SERVICE_KEY`, `SUPABASE_URL` and public Vite Supabase settings are configured in the deployment environment.
5. Test one-student/one-device locking with two browsers: the second login must be rejected while the first is active.
6. Test student logout, then confirm another browser can log in.
7. Test an abandoned session after the configured 15-minute idle period before relying on it for an exam.
8. Test practical file uploads and the PDF result export after deployment.
